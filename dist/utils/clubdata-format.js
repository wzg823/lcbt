...
// 俱乐部 首页
//0
export function formatClubList0(list, clubList) {
  list.forEach(item => {
    clubList.push({
      activeId: item.id,
      ...
      act_price:item.cards.act_price,
      price:item.cards.price,
      item:item
    });
  })
  return clubList
...
//1
export function formatClubList1(list, clubList) {
  list.forEach(item => {
    clubList.push({
      activeId: item.id,
      titleImg: item.pic_url,
